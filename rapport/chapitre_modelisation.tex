...
Les valeurs aberrantes sont traitées par un plafonnement fondé sur l'intervalle interquartile. Cette méthode utilise un facteur de 3,0 pour limiter le poids des observations manifestement aberrantes sans supprimer les véritables pics épidémiques. Dans le pipeline du projet, ce traitement a conduit à plafonner 5 valeurs de \texttt{TOTALCAS} sur l'agrégation hebdomadaire nationale, ce qui est cohérent avec une correction conservatrice des erreurs de saisie.

Les séries trop creuses sont également filtrées. Pour chaque maladie, le pipeline calcule la proportion de semaines avec zéro cas. Si cette proportion dépasse 70\%, la maladie est retirée du processus d'apprentissage, car ce profil traduit souvent une absence de signalement fiable plutôt qu'une dynamique épidémique exploitable. Dans le dataset 2023, cette règle a entraîné la suppression de 4 maladies : CHIKUNGUNYA, DIPHTERIE, DRACUNCULOSE et FHA, soit un passage de 27 à 23 maladies conservées.

Pour les maladies conservées, les petites interruptions de reporting sont comblées par interpolation linéaire sur les séquences de zéros isolées d'au plus deux semaines. Les zéros en début ou fin de série ne sont pas interpolés afin de ne pas inventer des observations en dehors de l'horizon disponible.

D'autres solutions auraient également été possibles : l'exclusion directe des observations aberrantes, le remplacement par des médianes ou des moyennes glissantes, ou le maintien de toutes les valeurs non filtrées. Le compromis retenu ici reste conservateur, car il préserve les tendances épidémiques tout en limitant l'impact des anomalies et des signaux de reporting erratiques.
...
\subsection{Classification du niveau d'alerte}

Au‑delà de la précision numérique des prévisions, la finalité opérationnelle de SAFE CONGO est de produire des classifications de sévérité exploitables par les autorités sanitaires. Pour chaque projection hebdomadaire, la valeur prédite (nombre de cas) est traduite en un niveau de sévérité \textit{Faible}, \textit{Modéré} ou \textit{Élevé} à l'aide de seuils établis de manière spécifique à chaque maladie.

Principe général de définition des seuils
\begin{itemize}
  \item Les seuils sont calculés par maladie à partir de l'historique hebdomadaire disponible (jeu 2022--2023 lorsque présent). Cette approche permet d'adapter les niveaux d'alerte à l'ordre de grandeur propre à chaque pathologie.
  \item On retient des repères robustes aux valeurs extrêmes : la médiane $Q_2$ (50\textsuperscript{e} percentile) et un quantile élevé (p.ex. $Q_{90}$ ou $Q_{95}$) servant de barrière vers le niveau \textit{Élevé}.
  \item Une règle d'accélération complète la logique seuil : une hausse relative importante (taux de croissance inter‑hebdomadaire supérieur à un seuil $g$) peut élever le niveau d'alerte même si la valeur absolue reste modérée.
\end{itemize}

Formulation mathématique (par maladie)
\[
\text{IQR} = Q_{3} - Q_{1},\qquad Q_{2} = \text{médiane},\qquad Q_{p} = \text{p\'ercentile } p.
\]
On définit typiquement :
\begin{align*}
\text{Faible} &:\quad x \le Q_{2},\\
\text{Mod\'er\'e} &:\quad Q_{2} < x \le Q_{p},\\
\text{\'Elev\'e} &:\quad x > Q_{p} \quad\text{ ou }\quad \frac{x - x_{-1}}{\max(1,x_{-1})} > g,
\end{align*}
où $x$ est la valeur pr\'edite pour la semaine cible, $x_{-1}$ la valeur de la semaine pr\'ec\'edente (ou une moyenne r\'ecente), $p$ un quantile choisi (souvent $90$ ou $95$) et $g$ un seuil de croissance relatif (p.ex. $g=0{,}5$ pour +50\%).

Tableau des règles synthétiques
\begin{table}[H]
  \centering
  \begin{tabular}{|p{3.6cm}|p{7.6cm}|p{4cm}|}
    \hline
    \textbf{Niveau} & \textbf{Condition (par maladie)} & \textbf{Action opérationnelle} \\
    \hline
    Faible & $x \le Q_{2}$ (médiane historique) & Surveillance standard ; pas d'alerte automatique \\
    \hline
    Modéré & $Q_{2} < x \le Q_{p}$ (p=90 ou 95) & Surveillance renforcée ; prévoir revue locale et vigilance accrue \\
    \hline
    Élevé & $x > Q_{p}$ \textit{ou} croissance hebdomadaire $> g$ (ex. 50\%) & Alerte : notification prioritaire, déclenchement procédure d'investigation \\
    \hline
  \end{tabular}
  \caption{Règles synthétiques de classification du niveau d'alerte}
  \label{tab:alerte_regles}
\end{table}

Exemples illustratifs (valeurs fictives montrant la mise en oeuvre)
\begin{table}[H]
  \centering
  \begin{tabular}{|l|c|c|c|p{4.5cm}|}
    \hline
    \textbf{Maladie} & $Q_{2}$ & $Q_{90}$ & $g$ & \textbf{Interpr\'etation} \\
    \hline
    Choléra & 8 cas & 35 cas & 0{,}5 & Faible $\le 8$, Mod\'er\'e 9--35, Élev\'e $>35$ ou +50\%/sem \\
    \hline
    Paludisme (confirmé) & 120 cas & 950 cas & 0{,}5 & Seuils supérieurs refl\'etent la forte incidence de fond \\
    \hline
    COVID‑19 & 15 cas & 120 cas & 0{,}7 & Croissance tr\`es importante (\'e.g. $g=70\%$) peut d\'eclencher une alerte \\
    \hline
  \end{tabular}
  \caption{Exemples de seuils par maladie (illustratif)}
  \label{tab:alerte_exemples}
\end{table}

Remarques pratiques
\begin{itemize}
  \item Les valeurs $Q_{2}$ et $Q_{p}$ sont recalculées périodiquement (p.ex. lors de chaque cycle d'entraînement) afin d'intégrer l'historique le plus récent.
  \item Un minimum absolu peut être imposé (p.ex. 1 cas) pour éviter des variations relatives extrêmes lorsque les séries ont beaucoup de zéros.
  \item Les seuils peuvent être affinés par maladie en concertation avec les experts métiers et en tenant compte de la gravité clinique et des capacités locales de réponse.
  \item La politique de seuils doit toujours être accompagnée d'un rapport indiquant le nombre d'alertes générées et les faux positifs/negatifs observés lors d'une période pilote.
\end{itemize}
