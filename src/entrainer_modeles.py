...
def _run_legacy_regression_training() -> bool:
    """Fallback: exécute l'ancien pipeline de régression par maladie."""
    root = Path(__file__).resolve().parent.parent
    ...
    try:
        ...
        print(f"❌ Erreur pendant l'entraînement legacy: {exc}")
        return False
...
def main():
    ...
    print("⚠️ train_run.log introuvable. Exécution du pipeline legacy en fallback...")
    if not _run_legacy_regression_training():
        print("❌ Impossible d'afficher le benchmark 81.6 ni d'entraîner en fallback.")
